...
metadata
{
     definition(name: "TH112xZB Sinope Thermostat EnergyMerter V4", namespace: "sacua", author: "Samuel Cuerrier Auclair") {
        ...
        preferences {
          ...
          input name: "tempChange", type: "number", title: "Minumum change of temperature reading to trigger report, 5..50", range: "5..50", defaultValue: 50
          input name: "MaxInterRepTemp", type: "number", title: "Maximum interval reporting time for temperature reading, 30..600", range: "30..600", defaultValue: 600
          ...
          input name: "HeatingChange", type: "number", title: "Heating change in % to trigger energy calculation, 1..25", range: "1..25", defaultValue: 1
          input name: "MaxInterRep", type: "number", title: "Maximum interval reporting time for energy calculation in seconds, 30..600", range: "30..600", defaultValue: 300
...
private createCustomMap(descMap){
    ...
    def map = [: ]
        if (descMap.cluster == "0201" && descMap.attrId == "0000") {
            ...
            runIn(1, "refreshPower") // Heating changed .. Request energyReport in 1 seconds
...
def configure(){    
    ...
    schedule(timesec + " " + timemin + " " + timehour + "/6 * * ? *",refreshTime) //refresh the clock at random begining and then every 6h
    ...
    // Configure Reporting
    cmds += zigbee.configureReporting(0x0201, 0x0000, 0x29, 1, (int) MaxInterRepTemp, (int) tempChange)     //local temperature
    cmds += zigbee.configureReporting(0x0201, 0x0008, 0x0020, 1, (int) MaxInterRep, (int) HeatingChange)    //PI heating demand
    ...
    //cmds += zigbee.configureReporting(0x0B04, 0x050B, DataType.INT16, 30, 599, 0x64) //Thermostat power draw - The thermostat does not seems to support automatic power report or the interval of reporting is too large
...
def setHeatingSetpoint(preciseDegrees) {
    if (preciseDegrees != null) {
        ...
        sendEvent(name: "thermostatSetpoint", value: celsius, unit: temperatureScale) //For interoperability with SharpTools 
...
private energyReport(value) {
    ...
    if (roundTwoPlaces(state.energyValue) != EnergyValue)
